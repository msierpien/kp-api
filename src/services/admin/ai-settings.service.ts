...
const defaults = {
  ...
  editorModel: null,
  editorDailyLimit: 50,
  editorPerCaseLimit: 10,
  editorSystemPrompt: null,
};
...
function toResponse(settings: any) {
  return {
    ...
    editor: {
      enabled: settings.editorEnabled ?? defaults.editorEnabled,
      provider: settings.editorProvider ?? null,
      model: normalizeAiModelId(settings.editorModel) ?? null,
      dailyLimit: settings.editorDailyLimit ?? defaults.editorDailyLimit,
      perCaseLimit: settings.editorPerCaseLimit ?? defaults.editorPerCaseLimit,
      systemPrompt: settings.editorSystemPrompt ?? null,
    },
    lastTest: {
